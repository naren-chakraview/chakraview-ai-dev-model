...
Replace `{script-name}`, `{service}`, `{input-files}`, and `{output-path}` before running.
...
- [ ] If a new contract artifact is introduced, `tooling/validate-contracts.sh` extended to validate it
